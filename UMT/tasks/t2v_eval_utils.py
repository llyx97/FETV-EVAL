...
def evaluation_wrapper(model, data_loader, tokenizer, device, config, prefix=""):
    with torch.cuda.amp.autocast(enabled=config.fp16):
        ...
        t2i_x, i2t_emb, t2i_emb = evaluation(
            model, data_loader, tokenizer, device, config
        )
    scores = {'UMTScore': {str(id): float(score) for id, score in enumerate(t2i_x.diagonal(0))}}
    avg_scores = {'UMTScore': t2i_x.diagonal(0).mean()}
...
def evaluation(model, data_loader, tokenizer, device, config):
    ...
    for i, sims in enumerate(iterator):
        ...
        topk_idx = torch.tensor([i]).to(sims.device)    # only conduct deep fusion for the same image/video-text pair

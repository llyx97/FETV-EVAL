...
from compression.compress import init_compression, redundancy_clean
...
def my_filling_sequence(
    ...
    tokens = tokens[..., :context_length]     # The input seq may contain mask tokens (id=-1)
    ...
    for fid in range(current_frame_num):
        input_tokens[:, text_len+400*fid] = tokenizer['<start_of_image>']  # add <SOI> to every current frame
        if guider_seq is not None:
    ...
    while counter < len(seq[0]) - 1:
        ...
        # token[:, index: counter+1] needs forwarding. ([index: counter+1] can be considered as the window size)
        if index == 0:
            ...
            for batch_idx in range(0, input_tokens.shape[0], group_size):
                ...
                for id, mem_kv in enumerate(mem_kv01):          # id in [0, 1]
...
def main(args):
    ...
    if args.stage_1 or args.both_stages:
        ############ Debug ############
        # model_stage1 = InferenceModel_Sequential(args)
        # model_stage1 = init_compression(model_stage1, args.deepspeed_config, teacher_model=None)
        ############ End Debug ############
    ...
    if args.stage_2 or args.both_stages:
        ############ Debug ############
        #model_stage2 = InferenceModel_Interpolate(args)
        #model_stage2 = init_compression(model_stage2, args.deepspeed_config, teacher_model=None)
        ############ End Debug ############
    ...
    if not args.stage_1:
        ...
        dsr = DirectSuperResolution(args, dsr_path,
                                    max_bz=2, onCUDA=False)
        # dsr = init_compression(dsr, args.deepspeed_config, teacher_model=None)
        ############ End Debug ############
    ...
    def process_stage2(model, seq_text, duration, video_raw_text=None, video_guidance_text="视频", parent_given_tokens=None, conddir=None, outputdir=None, gpu_rank=0, gpu_parallel_size=1):
        ...
        if args.both_stages or args.stage_2:
            move_start_time = time.time()
